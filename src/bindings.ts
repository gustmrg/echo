...
async getMeetingCapabilities() : Promise<MeetingCapabilities> {
    return await TAURI_INVOKE("get_meeting_capabilities");
},
async getMeetingCaptureDevices() : Promise<Result<MeetingCaptureDevice[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_meeting_capture_devices") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async setMeetingOutputDevice(deviceId: string | null) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("set_meeting_output_device", { deviceId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async getMeetingOutputDevice() : Promise<string | null> {
    return await TAURI_INVOKE("get_meeting_output_device");
},
async startMeeting() : Promise<Result<MeetingSummary, string>> {
    try {
...
async stopMeeting() : Promise<Result<MeetingSummary, string>> {
    try {
...
async getMeetingState() : Promise<Result<MeetingSummary | null, string>> {
    try {
...
async listMeetings(offset: number, limit: number) : Promise<Result<MeetingSummary[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("list_meetings", { offset, limit }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async getMeeting(id: number) : Promise<Result<MeetingDetail, string>> {
    try {
...
async retryMeetingTranscription(id: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("retry_meeting_transcription", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async renameMeeting(id: number, title: string) : Promise<Result<MeetingSummary, string>> {
    try {
...
async getMeetingAudioPaths(id: number) : Promise<Result<[string, string], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_meeting_audio_paths", { id }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
selected_channel?: number | null; clamshell_microphone?: string | null; selected_output_device?: string | null; 
/**
...
export type MeetingCapabilities = { supported: boolean; platform: string; minimum_os: string | null; reason: string | null }
export type MeetingCaptureDevice = { id: string; name: string; is_default: boolean }
export type MeetingDetail = { meeting: MeetingSummary; segments: MeetingSegment[] }
export type MeetingSegment = { id: number; meeting_id: number; source: MeetingSource; start_ms: number; end_ms: number; text: string }
export type MeetingSource = "microphone" | "system"
export type MeetingStateEvent = { meeting_id: number; status: MeetingStatus; phase: string; processed_ms: number; duration_ms: number; error: string | null }
export type MeetingStatus = "recording" | "transcribing" | "completed" | "failed" | "interrupted"
export type MeetingSummary = { id: number; title: string; started_at: number; ended_at: number | null; duration_ms: number; status: MeetingStatus; transcript_text: string; error_message: string | null; dropped_microphone_packets: number; dropped_system_packets: number }
