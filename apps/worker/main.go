...
import (
	...
	"github.com/gustmrg/echo/apps/worker/internal/transcription"
)
...
func main() {
	...
	transcriber, err := transcription.NewTranscriber(cfg.Transcription)
	if err != nil {
		log.Fatal(err)
	}
	...
	for {
		...
		processJob(ctx, store, storage, transcriber, job)
	}
}

func processJob(ctx context.Context, store *db.Store, storage *storage.S3Storage, transcriber transcription.Transcriber, job *db.TranscriptionJob) {
	...
	transcript, err := transcriber.TranscribeAudio(ctx, bytes.NewReader(file), recording.FileName, contentType)
	if err != nil {
